    ...
    <div class="min-h-screen flex flex-col">
        ...
        <div class="mx-auto w-full md:w-5/12 ">
            <div class="w-full text-xl text-black bg-gray-300 py-2 px-6 uppercase">
                ...
                <div class="flex"><span
                        class="w-2/5 font-semibold">LK tác nghiệp: </span><span>{{$plan->sltacnghiep}}</span></div>
                <div class="flex"><span
                        class="w-2/5 font-semibold">LK thực hiện: </span><span>{{$plan->thuchien}}</span></div>
            </div>
            <form method="POST" action="{{ route('produce.editWarehouseUpdate',$plan) }}" class="p-4 border">
                ...
                <div class="my-5 flex items-center gap-8">
                    ...
                    <input type="number" id="tacnghiepmoi" name="tacnghiepmoi" min="{{$plan->thuchien-$plan->sltacnghiep}}"
            ...
            <form method="POST" action="{{ route('produce.editWarehouseUpdate',$plan) }}" class="p-4 border">
                ...
                <div class="my-5 flex items-center gap-8">
                    ...
                    <input type="number" name="nhaphoanthanh" max="{{$plan->thuchien}}" value="{{$plan->nhaphoanthanh}}"
                           class="flex-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5"/>
                </div>
                <div class="flex justify-center">
                    <button type="submit"
