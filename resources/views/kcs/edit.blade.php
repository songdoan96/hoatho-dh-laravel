...
@section('content')
    <div class="flex justify-center items-center w-screen min-h-screen bg-black text-white overflow-x-hidden">
        <div class="w-full h-full md:w-1/2 p-8">
            <a href="{{route('kcs.dashboard')}}" class="flex items-center justify-center">
                <img src="{{asset('images/logo2.png')}}" alt="Logo" width="300">
            </a>
            <h1 class="bg-blue-500 text-center text-3xl font-bold py-1 my-2">BÁO CÁO KCS {{$kcs->plans->chuyen}}</h1>
            <h1 class="font-bold text-center text-2xl">{{formatDate($kcs->ngaytao,"d/m/Y")}}
                - {{$kcs->plans->khachhang}}
                - {{$kcs->plans->mahang}}</h1>
            <div class="flex my-4">
                ...
                <div class="w-1/3 flex flex-col items-center justify-center">
                    <p>Tỷ lệ lỗi</p>
                    <p class="text-3xl font-bold"> @if($kcs->sldat==0 && $kcs->slloi==0)
                            0%
                        @else
            ...
            <div class="flex flex-col justify-center items-center my-2">
                ...
                <form method="post" action="{{route('kcs.updateErrorInfo',$kcs)}}" class="flex flex-col w-full">
                    @csrf
                    <textarea name="chitietloi" class="w-full text-black p-2" rows="3">{{$kcs->chitietloi}}</textarea>
                    <div class="flex justify-center w-full gap-2 my-2">
