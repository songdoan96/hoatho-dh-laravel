    ...
    <div class="min-h-screen flex flex-col ">
        <div id="header" class="flex items-center px-2 py-1 bg-blue-500 text-white">
            ...
            <div class="flex flex-1 justify-between items-center">
                <h1 class="text-2xl uppercase font-bold text-center w-full">KCS {{$xn}} - thêm chỉ tiêu ngày <span class="font-bold text-black bg-white px-1">{{date("d-m-Y")}}</span></h1>
            </div>
        </div>
        <div class="w-full md:w-1/3 mx-auto my-2 text-sm text-black bg-orange-200 py-2 px-6">
            <ul >
        ...
        <div class="flex justify-center flex-col items-center">
            <form method="post" action="{{route('kcs.store')}}" class="w-full md:w-1/3 border shadow-lg p-4">
                @csrf
